"""
Meeting Bot Service - Domain Layer.
...
from app.infrastructure.email_service import CombinedEmailService
from app.infrastructure.scheduler import MeetingScheduler
from app.infrastructure.meeting_handler import MeetingJoiner
from app.domain.models import MeetingDetails, MeetingSession, TranscriptSegment, MeetingPlatform, MeetingSource

main_logger = get_logger("meeting_bot_service")
...
class MeetingBotService:
    ...
    def __init__(self):
        """Initialize the Meeting Bot Service."""
    ...
    async def initialize(self) -> bool:
        ...
        """
        main_logger.info("Initializing Meeting Bot Service...")
        
        # Note: Auth server is now handled by FastAPI app, not here
        ...
        if not auth_success:
            ...
            main_logger.info("=" * 60)
            main_logger.info(f"✨ Auth UI: http://{settings.auth_server.host}:{settings.auth_server.port}/api/v1/auth")
        ...
        main_logger.info("Meeting Bot Service initialized successfully")
        self._auth_only_mode = False
        return True
    
    # Note: run() method removed - FastAPI app lifecycle handles startup/shutdown
    # Scheduler is started in initialize(), FastAPI handles the event loop
    
    async def manual_join(self, bot_name: str, meeting_url: str) -> dict:
        """
...
# Service class ends here - FastAPI app handles lifecycle
