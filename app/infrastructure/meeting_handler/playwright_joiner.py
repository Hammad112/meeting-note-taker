...
from app.domain.models.meeting import MeetingDetails, MeetingPlatform
from app.infrastructure.transcription.service import TranscriptionService
        ...
        try:
            while True:
                ...
                try:
                    ...
                    # 3. For Google Meet, use keyboard shortcut 'c' (more reliable than clicking)
                    # Check if this is Google Meet by looking at the URL
                    url = page.url
                    if "meet.google.com" in url:
                        logger.info("Google Meet detected - using keyboard shortcut 'c' to enable captions...")
                        ...
                        turn_off_btn = page.locator('button[aria-label*="Turn off captions"]')
                        if await turn_off_btn.count() > 0 and await turn_off_btn.first.is_visible():
                            logger.info("✅ Successfully enabled captions via keyboard shortcut.")
                            break
                        else:
                            logger.debug("Pressed 'c' but caption status unclear, will retry...")
                    
                    # 4. For other platforms, try clicking the button
                    elif found_button and target_btn:
                    ...
                    # 5. If NO button found (non-Google Meet platforms), try "More Options" menu
                        ...
                        if await more_options.count() > 0 and await more_options.first.is_visible():
                            ...
                            logger.debug("No caption controls found.")
