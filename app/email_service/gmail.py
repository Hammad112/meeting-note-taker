...
from app.config import settings, get_logger, AuthMethod
...
class GmailService(EmailServiceBase):
    ...
    async def authenticate(self) -> bool:
        ...
        if auth_method == AuthMethod.CREDENTIALS:
            ...
            logger.error(f"   Authenticate at: {settings.backend.url}/auth/gmail/start")
            return False
        ...
        if auth_method in [AuthMethod.AUTO, AuthMethod.OAUTH]:
            ...
            logger.error(f"   Please authenticate at: {settings.backend.url}/auth/gmail/start")
            return False
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    ...
                    logger.error(
                        ...
                        f"Visit: {settings.backend.url}/auth/gmail/start"
                    )
                    return False
